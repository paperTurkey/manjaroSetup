...
if [ ! -e "/usr/bin/ManjaroSetup" ];then
    ...
    if [[ $install = Y || $install = y ]] ; then
        sudo wget https://raw.githubusercontent.com/paperTurkey/manjaroSetup/master/manjaroSetup.sh -O /usr/bin/ManjaroSetup
        sudo chmod +x /usr/bin/ManjaroSetup
...
function installvirtualbox {
  ...
  sudo pacman -S --noconfirm virtualbox qt5ct
  echo && echo -e "${b}${enda} was successfully installed."
  echo -en " ${y}Press Enter to return to menu${endc}"
  echo
  read input
